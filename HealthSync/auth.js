const REGISTERED_USERS_KEY = "healthsync-registered-users";
(function () {
  ...
  const users = JSON.parse(localStorage.getItem("healthsync-registered-users")) || [];
 ...
 document.addEventListener("DOMContentLoaded", () => {
  ...
  if (page === "register") {
    initRegisterPage(); // ✅ THIS WAS MISSING
    return;
  }
...
function initRegisterPage() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const existing = getRegisteredUsers().find(u => u.username === username);
    if (existing) {
