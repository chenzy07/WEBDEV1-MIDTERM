(function () {
  const SESSION_KEY = "courtsidehoops-portal-session";
  ...
  const accounts = [
    {
      ...
      description: "Main Control and Analytics."
    },
    {
      role: "staff",
      label: "Staff",
      ...
      description: "Coordination for daily operations."
    },
    {
      role: "user",
      label: "User",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!profileMenu || !btn || !dropdown) return;
  ...
  // logout (clean instant logout)
  document.querySelectorAll("[data-logout]").forEach((logoutBtn) => {
    logoutBtn.addEventListener("click", (e) => {
      e.stopPropagation();

      localStorage.removeItem("courtsidehoops-session");
      localStorage.removeItem("courtsidehoops-portal-session");
